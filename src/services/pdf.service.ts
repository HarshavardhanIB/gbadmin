...
import {SERVER_FILES_PATH, SERVER_FOLDER} from '../paths';
import {createFilex} from '../storage.helper';
...
export class PdfService {
   ...
   */
  async createNewPdfFile(filename: string) {
  }
  ...
  async fillForm(data: any) {
    const formUrl = SERVER_FILES_PATH + 'PAD_Agreement.pdf' //'https://pdf-lib.js.org/assets/dod_character.pdf'
    const formPdfBytes = await fetch(formUrl).then((res: any) =>
      res.arrayBuffer()
      //res.buffer()
    )

    const marioUrl = 'https://pdf-lib.js.org/assets/small_mario.png'
    const marioImageBytes = await fetch(marioUrl).then((res: any) => res.arrayBuffer())

    const emblemUrl = 'https://pdf-lib.js.org/assets/mario_emblem.png'
    const emblemImageBytes = await fetch(emblemUrl).then((res: any) => res.arrayBuffer())
    ...
    if (saved) {
      return SERVER_FILES_PATH + filename
    } else {
      return "Not saved";
    }
  ...
  async modifyPdf(data: any) {
    //const url = 'https://pdf-lib.js.org/assets/with_update_sections.pdf'
    const url = SERVER_FILES_PATH + 'PAD_Agreement.pdf' //'https://pdf-lib.js.org/assets/dod_character.pdf'
    ...
    if (data.voidChequeType == "pdf") {
      ...
      const voidChequePDFBytes = await fetch(voidChequePDFUrl).then((res: any) => res.arrayBuffer())
      ...
      const voidChequeImageBytes = await fetch(voidChequeImageUrl).then((res: any) => res.arrayBuffer())
    ...
    let today = moment().format('DD-MM-YYYY'); //or enrollmentDate??
  ...
  async addCustomerDataPdf_v1(data: any) {

    const url = SERVER_FILES_PATH + 'PAD_Agreement.pdf'
    const existingPdfBytes = await fetch(url).then(res => res.arrayBuffer())
    ...
    const FULL_WHITE = rgb(255 / 255, 255 / 255, 255 / 255);
    ...
    if (data.voidChequeType == "pdf") {
      ...
      secondPage.drawPage(chequePDF, {
        ...
        //xScale: xScale,
        yScale: yScale,
        width: (adjustedChequeWidth - 1),//(580.5 - 42.5),
    ...
    if (saved) {
      //return SERVER_FOLDER + "/" + data.filename + ".pdf"
      return SERVER_FILES_PATH + data.filename + ".pdf"
    } else {
      return "Not saved";
    }
  }

  async addCustomerDataPdf(data: any) {
    //v2 new letter sized template

    const url = SERVER_FILES_PATH + 'PAD_Agreement_Letter.pdf'
    const existingPdfBytes = await fetch(url).then(res => res.arrayBuffer())
  ...
  x1,y1-----------------------x2,y1
    ...
    */
    const linkAreaPosition: any = {

      x1: 300 - 2, // lower left x coord
      y1: height2 - 373 - 3, // lower left y coord
      x2: 435, // upper right x coord
  ...
  async addSignaturePdf_v1(data: any) {
    //const url = 'https://pdf-lib.js.org/assets/with_update_sections.pdf'
  ...
  async addSignaturePdf(data: any) {
  ...
  async savePDF(pdfBytes: any, filename: string) {
    try {

      await createFilex(SERVER_FOLDER, filename, pdfBytes);
      return true;
      // fos.flush();
    } catch (error) {
  ...
  createPageLinkAnnotation = (pdfDoc: PDFDocument, pageRef: PDFRef, position: any) =>
    pdfDoc.context.register(
      pdfDoc.context.obj({
        ...
        Rect: [
          position.x1,// 145, // lower left x coord
          position.y1, //height - 200 - 10, // lower left y coord
          position.x2,//358, // upper right x coord
  ...
  // createPageLinkAnnotation = (page: PDFPage, uri: string) =>
