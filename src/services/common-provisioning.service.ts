...
export class CommonProvisioningService {
  getPackageValueExecutive //SP
    (equitablePackages: any, stateId: any) {
   ...
   */
  getPackageValue(greenshieldPackages: any[], stateId: any) {
  ...
  getPackageValueGreenshield(greenshieldPackages: any[], stateId: any) {
  ...
  getPackageValueEquitable(equitablePackages: any[], stateId: any) {
    ...
    for (const esp of equitablePackages) {
      if (esp.stateId == parseInt(stateId)) {
        ...
        espDivisionName = esp.divisionName?.split(" - ")[0]?.trim();
        //D - outside BC/MB/SK who selected GoldPlan (Div2)
        espClassName = esp.className?.split(" - ")[0]?.trim();
  ...
  getGenderGreenshield(gender: string) {
  ...
  getCoverageValue(greenshieldPlan: any) {
  ...
  getCoverageValueGreenshield(greenshieldPlan: any) {
  ...
  getGenderEquitable(gender: string) {
    let equitableGender;
    switch (gender) {
      case GENDER.MALE:
        equitableGender = '2'
  ...
  getCoverageValueEquitable(equitablePlan: any) {
  ...
  getYearMonthDateSplit(date: string) {
    let dateObj: any = {}
  ...
  async getDependantInfo(customerRelatives: any) {

    //single dependant -- spouse or child

    let dependantsExtdataObject: any = {}
    ...
    let schoolChildren: any = []
    let disabledChildren: any = []
    ...
    for (let cr of customerRelatives) {
      ...
      if (cr.relationshipType == "CHILDREN") {
        ...
        if (cr.isDisabled) {
          ...
          dependantsExtdataObject["Dependents-s2-req" + index++] = moment(cr.dob).format('YYYY')
          dependantsExtdataObject["Dependents-s2-req" + index++] = moment(cr.dob).format('MM')
          dependantsExtdataObject["Dependents-s2-req" + index++] = moment(cr.dob).format('DD')
  ...
  async getDependantsInfo(customerRelatives: any) {
    ...
    for (let cr of customerRelatives) {
      ...
      if (cr.relationshipType == "CHILDREN") {
        ...
        if (cr.isDisabled) {
          ...
          dependantsExtdataObject["Dependents-s5-req" + index++] = moment(cr.dob).format('YYYY')
          dependantsExtdataObject["Dependents-s5-req" + index++] = moment(cr.dob).format('MM')
          dependantsExtdataObject["Dependents-s5-req" + index++] = moment(cr.dob).format('DD')
  ...
  async getDependantInfoGreenshield(customerRelatives: any) {
  ...
  async getDependantsInfoGreenshield(customerRelatives: any) {
  ...
  async getDependantInfoEquitable(customerRelatives: any) {
    // {
    ...
    for (let cr of customerRelatives) {
      ...
      dependantsExtdataObject["PersonalInfo-s2-req25"] = moment(cr.dob).format('M/D/YYYY')
      ...
      if (cr.relationshipType == "CHILDREN") {
        ...
        if (cr.isDisabled) {
          ...
          if (childAge >= 21) { //21 for equitable
            ...
            //   dependantsExtdataObject["Dependents-s3-req1"] = 'N'
            // }
            dependantsExtdataObject["PersonalInfo-s2-req29"] = moment(cr.universityGraduationDay).format('M/D/YYYY')
          } else {
        ...
        dependantsExtdataObject["PersonalInfo-s2-req33"] = (cr.cobCoverage?.toLowerCase() == "couple" || cr.cobCoverage?.toLowerCase() == "family") ? "2" : "1" //SINGLE always //3
      } else {
        console.log(cr.relationshipType)
      }
      ...
      dependantsExtdataObject["PersonalInfo-s2-req34"] = cr.carrierName ?? ''
  ...
  async getDependantsInfoEquitable(customerRelatives: any) {
    ...
    for (let cr of customerRelatives) {
      ...
      dependantsExtdataObject["PersonalInfo-s2-req" + indexs2++] = moment(cr.dob).format('M/D/YYYY') //25
      ...
      if (cr.relationshipType == "CHILDREN") {
        ...
        if (cr.isDisabled) {
          ...
          if (childAge >= 21) { //21 for eq.
            ...
            dependantsExtdataObject["PersonalInfo-s2-req"] = moment(cr.universityGraduationDay).format('M/D/YYYY') //29 //coverageExpiry/unviersaty lastdate
          } else {
        ...
        dependantsExtdataObject["PersonalInfo-s2-req" + indexs2++] = no_of_dependants + 1; //31
        dependantsExtdataObject["PersonalInfo-s2-req" + indexs2++] = '50' //32 //all benefits
        dependantsExtdataObject["PersonalInfo-s2-req" + indexs2++] = (cr.cobCoverage?.toLowerCase() == "couple" || cr.cobCoverage?.toLowerCase() == "family") ? "2" : "1" //SINGLE always //3
      ...
      dependantsExtdataObject["PersonalInfo-s2-req" + indexs2++] = cr.carrierName ?? '' //34
...
[
  ...
  */
  ...
  getCoverageValueExecutive(executivePlan: any) {
    let coverage;
...
function calculateAge(dob: any) {
  ...
  return moment().diff(dob, 'years', false); //years
