...
import {aitp, EQUITABLE_REGISTRATION_STATUS, GREENSHEILD_REGISTRATION_STATUS, serviceEndpoints, userCreds} from '../constants';
import {CustomerRepository} from '../repositories';
import {HttpService} from './http.service';

const server = aitp.server

@injectable({scope: BindingScope.TRANSIENT})
export class AitpService {
  ...
  async signin() {
    ...
    const service = server + serviceEndpoints.login// '/api/auth/signin'

    const data = {
      username: userCreds.username,//'Tracer',
      password: userCreds.password//'Ideabytes@123'
    }
  ...
  async getExecutionId(token: string) {

    const service = server + serviceEndpoints.executionId// '/api/user/get_execution_id'
  ...
  async executeWithExternalData(data: any, token: string) {

    const service = server + serviceEndpoints.externalData // '/api/user/execute_script_external_source_multiple'
  ...
  async mainExecution(data: any) {

    let singleExecResult: any
    let coupleExecResult: any
    let familyExecResult: any
  ...
  async soleExecution(data: any, registartion: boolean, service: string) {
    //signing
    const user = await this.signin();
  ...
  async checkLogDataGS(data: any, customerId: number, registration: boolean, coverage: string) {
    //[]
    /*[
      {
        logType: 'INFO ',
    ...
    if (registration) {
      const results = data.filter((obj: any) => {
      ...
      if (greenshieldMemberId) {
        await this.customerRepo.updateById(customerId, {
          ...
          greenshieldRegistrationStatus: GREENSHEILD_REGISTRATION_STATUS.DONE
        })
      }
    } else {
...
2022-08-05 15:31:53.474 | ERROR | IBSeleniumScripter:3265 | Personal_Info | Dataset_2 | Step_5 | Response_Item_5 | Get text `Aug 4, 2022` details | Compare: #Aug5,2022# && #Aug 4, 2022# by xpath:/html/body[1]/form[1]/table[2]/tbody/tr/td[2]/table[3]/tbody/tr[3]/td[2]/span >> Issue with spelling or spaces(tabs, newlines) or unico
  ...
  async getMemebershipId(response: any) {
  ...
  /***EQ */
  async checkLogDataEQ(data: any, customerId: number, registration: boolean, coverage: string) {
    //[]
    /*[
      {
        logType: 'INFO ',
    ...
    if (registration) {
      ...
      if (equitableCertId) {
        await this.customerRepo.updateById(customerId, {
          ...
          equitableRegistrationStatus: EQUITABLE_REGISTRATION_STATUS.DONE
        })
      }
    } else {
...
2022-08-05 15:31:53.474 | ERROR | IBSeleniumScripter:3265 | Personal_Info | Dataset_2 | Step_5 | Response_Item_5 | Get text `Aug 4, 2022` details | Compare: #Aug5,2022# && #Aug 4, 2022# by xpath:/html/body[1]/form[1]/table[2]/tbody/tr/td[2]/table[3]/tbody/tr[3]/td[2]/span >> Issue with spelling or spaces(tabs, newlines) or unico
  ...
  async checkEQterm(responses: any) {
  ...
  async checkLogDataEXEC(data: any, datasets: number, registration: boolean, coverage: string) {
    ...
    let executiveRegistrationDatasets: any = {}

    let executiveTermination = true;
    let executiveTerminationDatasets: any = {}
    ...
    if (registration) {
      ...
      for (let ds = 2; ds <= (datasets + 1); ds++) {
        dataset = ds;
        const resultsT = data.filter((obj: any) => {
          return obj.testcase == testcase && obj.dataset == ('Dataset_' + ds) && obj.step == step && obj.item == 'Response_Item_1';
        });
